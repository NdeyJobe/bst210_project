...
world_data$pm_ordinal <- factor(world_data$pm_category, levels = levels_order, ordered = TRUE)
...
```{r}
# Assuming you have a data frame named 'your_data' with a 'mortality' variable
world_data <- world_data %>%
...
# Convert the variable to a factor
world_data$Mortality_Ordinal <- factor(world_data$Mortality_Category, levels = levels_order, ordered = TRUE)
```
...
world_data <- world_data %>% filter(GDP < 33500)

# Removes rows with missing values
world_data <- na.omit(world_data)

# Save your data frame to a CSV file
write.csv(world_data, "final_bst210_project_data.csv", row.names = FALSE)
```
